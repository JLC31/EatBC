...
class _CafeMenuState extends State<CafeMenu> {
  ...
  void getcafeItems() {
    ...
    {
      if (data[i]['Location_Number'] == '26' && data[i]['Meal_Name'] == 'Breakfast' && data[i]['Menu_Category_Name'] != 'GET Offerings (as available)')
      {
        cafeBItems.add(data[i]['Recipe_Print_As_Name']);
        cafeBFacts.add(data[i]);
      }
      else if (data[i]['Location_Number'] == '26' && data[i]['Meal_Name'] == 'Lunch' && data[i]['Menu_Category_Name'] != 'GET Offerings (as available)'){
        cafeLItems.add(data[i]['Recipe_Print_As_Name']);
        cafeLFacts.add(data[i]);
      }

    }

    setState(() {
      cafeBMenu = cafeBItems;
      cafeLMenu = cafeLItems;
  ...
  List mealName = ['Breakfast', 'Lunch'];
  
  
  @override
  Widget build(BuildContext context) {
    List mealItems = [cafeBMenu, cafeLMenu];
    String mealName = '';
