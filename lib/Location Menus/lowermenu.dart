...
class LowerMenu extends StatefulWidget {
  @override
  _LowerMenuState createState() => _LowerMenuState();
}

class _LowerMenuState extends State<LowerMenu> {
  ...
  void getLowerItems() {
    ...
    {
      if (data[i]['Location_Name'] == 'Lower Live/Heights Room') {
        meals.add(data[i]['Meal_Name']);
      }
      if (data[i]['Location_Name'] == 'Lower Live/Heights Room' && data[i]['Meal_Name'] == 'BREAKFAST' && data[i]['Menu_Category_Name'] != 'GET Offerings (as available)')
      {
        lowerBItems.add(data[i]['Recipe_Print_As_Name']);
        lowerBFacts.add(data[i]);
      }
      else if (data[i]['Location_Name'] == 'Lower Live/Heights Room' && data[i]['Meal_Name'] == 'LUNCH' && data[i]['Menu_Category_Name'] != 'GET Offerings (as available)'){
        lowerLItems.add(data[i]['Recipe_Print_As_Name']);
        lowerLFacts.add(data[i]);
      }
      else if (data[i]['Location_Name'] == 'Lower Live/Heights Room' && data[i]['Meal_Name'] == 'DINNER' && data[i]['Menu_Category_Name'] != 'GET Offerings (as available)'){
        lowerDItems.add(data[i]['Recipe_Print_As_Name']);
        lowerDFacts.add(data[i]);
      }
      else if (data[i]['Location_Name'] == 'Lower Live/Heights Room' && data[i]['Meal_Name'] == 'LATE NIGHT' && data[i]['Menu_Category_Name'] != 'GET Offerings (as available)'){
        lowerLNItems.add(data[i]['Recipe_Print_As_Name']);
        lowerLNFacts.add(data[i]);
      }

    }

    setState(() {
      lowerBMenu = lowerBItems;
      lowerLMenu = lowerLItems;
      lowerDMenu = lowerDItems;
      lowerLNMenu = lowerLNItems;

      lowerBNut = lowerBFacts;
      lowerLNut = lowerLFacts;
      lowerDNut = lowerDFacts;
      lowerLNNut = lowerLNFacts;
  ...
  void _showDialog (String foodItem, int index, int itemIndex) async {
  ...
  Widget build(BuildContext context) {
    List mealItems = [lowerBMenu, lowerLMenu, lowerDMenu, lowerLNMenu];
    String mealName = '';
    switch(_pageState){
      case 0:
        mealName = 'Breakfast';
